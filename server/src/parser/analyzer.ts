...
import { AqlLexer } from './lexer';
import { getAqlParser } from './parser';
import { ParseResult, ParseError, DocumentSymbols, SymbolInfo, Location } from './types';

/**
 * Analyzes AQL source code and extracts parse results and symbol information
 */
export class AqlAnalyzer {
  /**
   * Parse AQL source code and return parse result with errors
   */
  parse(source: string): ParseResult {
    // Tokenize
    const lexResult = AqlLexer.tokenize(source);
  ...
  getTokenAtPosition(tokens: IToken[], line: number, column: number): IToken | null {
    for (const token of tokens) {
      ...
      if (line >= token.startLine && line <= token.endLine) {
        if (line === token.startLine && column < token.startColumn) continue;
        if (line === token.endLine && column > token.endColumn) continue;
        return token;
      }
    }
    return null;
   ...
   */
  private findVariableReferences(tokens: IToken[], symbols: DocumentSymbols): void {
  ...
  private tokenToLocation(token: IToken): Location {
    return {
      start: {
  ...
  private convertLexerError(error: ILexingError): ParseError {
    return {
  ...
  private convertParserError(error: IRecognitionException): ParseError {
...
// Singleton instance
export const analyzer = new AqlAnalyzer();
